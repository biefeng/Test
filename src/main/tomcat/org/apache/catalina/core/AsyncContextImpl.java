...
public class AsyncContextImpl implements AsyncContext, AsyncContextCallback {
        ...
        try {
            ...
            for (AsyncListenerWrapper listener : listenersCopy) {
                try {
                    ...
                    log.warn("onComplete() failed for event of type [" +
            ...
            try {
                ...
                for (AsyncListenerWrapper listener : listenersCopy) {
                    try {
                        ...
                        ExceptionUtils.handleThrowable(t);
                        log.warn("onTimeout() failed for event of type [" +
                                listener.getClass().getName() + "]", t);
                    }
                }
        ...
        synchronized (asyncContextLock) {
            ...
            for (AsyncListenerWrapper listener : listenersCopy) {
                try {
                    ...
                    ExceptionUtils.handleThrowable(t);
                    log.warn("onStartAsync() failed for event of type [" +
                            listener.getClass().getName() + "]", t);
                }
            }
        }
    ...
    public void setErrorState(Throwable t, boolean fireOnError) {
        ...
        if (fireOnError) {
            ...
            for (AsyncListenerWrapper listener : listenersCopy) {
                try {
                    ...
                    log.warn("onError() failed for event of type [" +
        ...
        if (result.get()) {
            // No event called dispatch() or complete(). This is an error.
